...
const Reports = ({navigation, route}) => {
    ...
    const updateValue = (value) => {
        ...
        getList(value, selectedCom)
    }
    const updateCom = (com) => {
        setSelectedCom(com)
        getList(selectedValue, com)
    }

    const getList = (value, com) => {
        if(com == ""){
            return false;
        }
        if(value == ""){
        ...
        p_data.append("comm_type", com);
        p_data.append("period_from", value);
        fetch(api_url + "member_transaction_api/viewBonusHistory", {
            method: "POST",
            body: p_data,
            headers: new Headers({
    ...
    return (
        <View style={{flex: 1, height: "100%"}}>
            ...
            >
                <HeaderMenu navigation={navigation} back="Reports" title="Bonus Report"/>
                <View style={{paddingBottom: 30}}>
                    ...
                    <View style={{flex: 1, flexDirection: 'row', flexWrap: "wrap", marginTop: 10, marginHorizontal: 15, backgroundColor: COLOR.black, borderRadius: 5, padding: 10}}>
                        <View style={{width: "100%"}}>
                            <Picker style={{width: "100%", color: "white", height: 28, backgroundColor: COLOR.sky}} itemStyle={{height: 28}}
                                selectedValue={selectedValue}
                                onValueChange={(itemValue, itemIndex) => {
                                    updateValue(itemValue)
                                }}
                            >
                                <Picker.Item color={COLOR.header}key="-1" value="" label="Please Period" />
                    ...
                    <View style={{flex: 1, flexDirection: 'row', flexWrap: "wrap", marginTop: 5, marginHorizontal: 15, backgroundColor: COLOR.black, borderRadius: 5, padding: 10}}>
                        <View style={{width: "100%"}}>
                            <Picker
                                style={{ width: "100%", color: "white", height: 28, backgroundColor: COLOR.sky}}
                                itemStyle={{height: 28}}
                                selectedValue={selectedCom}
                                onValueChange={(itemValue, itemIndex) => {
                                    updateCom(itemValue)
                                }}
                            >
                                <Picker.Item color={COLOR.header}key="-1" value="" label={trans("PleaseSelect")} />
                                <Picker.Item color={COLOR.header}value="COM_REFERRAL_TOPUP_COMISSION" label="ec3030 Bonus" />
                                <Picker.Item color={COLOR.header}value="COM_REFERRAL_COMISSION" label="10 Level Bonus" />
                            </Picker>
                        </View>
                    </View>
                                    ...
                                    >
                                        <View style={{flex: 1, flexDirection: 'row', flexWrap: "wrap", marginTop: 10, marginHorizontal: 5, backgroundColor: COLOR.primary_color, borderRadius: 5, padding: 10}}>
                                            ...
                                            <View style={{alignSelf: "flex-start", width: "32%", marginBottom: 10}}>
                                                ...
                                                <Text style={{fontSize: 13, color: COLOR.white, marginBottom: 10, textAlign: 'center'}}>{item.from_member_id}</Text>
                                            </View>
                                            <View style={{alignSelf: "flex-start", width: "32%", marginBottom: 10}}>
                                                <Text style={{fontSize: 13, color: COLOR.white, marginBottom: 10, textAlign: 'center'}}>{trans('CreditIn')}</Text>
                                                <Text style={{fontSize: 13, color: COLOR.white, marginBottom: 10, textAlign: 'center'}}>{item.amount}</Text>
                                            </View>
                                            <View style={{alignSelf: "flex-start", width: "32%", marginBottom: 10}}>
                                                <Text style={{fontSize: 13, color: COLOR.white, marginBottom: 10, textAlign: 'center'}}>{trans('CreditOut')}</Text>
                                                <Text style={{fontSize: 13, color: COLOR.white, marginBottom: 10, textAlign: 'center'}}>0.00</Text>
                                            </View>
                                            <View style={{alignSelf: "flex-start", width: "32%", marginBottom: 10}}>
                                                <Text style={{fontSize: 13, color: COLOR.white, marginBottom: 10, textAlign: 'center'}}>{trans('Balance')}</Text>
                                                <Text style={{fontSize: 13, color: COLOR.white, marginBottom: 10, textAlign: 'center'}}>{trans('Success')}</Text>
                                            </View>
                                        </View>
                                    </TouchableHighlight>
                                )}
